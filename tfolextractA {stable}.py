...
class TelegramDataExtractor:
    ...
    async def _extract_folders(self):
        ...
        for folder in selected_folders:
            logging.info(f"Processing folder: {folder.title}...")

            pinned_names = await self._get_names_for_peers(folder.pinned_peers)
            included_names = await self._get_names_for_peers(folder.include_peers)
            excluded_names = await self._get_names_for_peers(folder.exclude_peers)

            extracted_data.append({
                'folder_name': folder.title.text, 'folder_id': folder.id, 'emoticon': folder.emoticon or "None",
                'pinned_chats': pinned_names,
                'included_chats': included_names,
                'excluded_chats': excluded_names,
